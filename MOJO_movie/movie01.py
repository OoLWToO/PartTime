...
if __name__ == '__main__':
    ...
    for year in range(2009, 2020):
        if year == 2009:
            ...
            worksheet1.title = f'{year}'
        else:
            worksheet1 = workbook1.create_sheet(title=f'{year}')
        worksheet1.cell(row=1, column=1, value='排名')
        worksheet1.cell(row=1, column=2, value='电影名')
        worksheet1.cell(row=1, column=3, value='全球')
        worksheet1.cell(row=1, column=4, value='国内')
        worksheet1.cell(row=1, column=5, value='国外')
        rows = 2
        detail_url = []
        # 请求地址
        ...
        for movie in movie_list:
            rank = movie.xpath('./td[1]/text()')[0]
            release_group = movie.xpath('./td[2]/a/text()')[0]
            worldwide = movie.xpath('./td[3]/text()')[0]
            domestic = movie.xpath('./td[4]/text()')[0]
            foreign = movie.xpath('./td[6]/text()')[0]
            detail_url.append(movie.xpath('./td[2]/a/@href')[0])
            print(f'{rank}   {release_group}   {worldwide}   {domestic}   {foreign}')
            worksheet1.cell(row=rows, column=1, value=rank)
            worksheet1.cell(row=rows, column=2, value=release_group)
            worksheet1.cell(row=rows, column=3, value=worldwide)
            worksheet1.cell(row=rows, column=4, value=domestic)
            worksheet1.cell(row=rows, column=5, value=foreign)
            rows += 1
        workbook1.save('2009年-2019年top200影片.xlsx')
