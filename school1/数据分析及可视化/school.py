...
if __name__ == '__main__':
    if not os.path.exists('图片'):
        os.makedirs('图片')
    df = pd.read_csv('../爬虫/数学与计算机专业分数线数据统计.csv')
    score_hebei = {}
    score_beijing = {}
    score_tianjin = {}
    score_985 = {}
    ...
    for index, row in df.iterrows():
        ...
        if row['所在地区'] == '天津':
            if not score_tianjin.get(row['专业名称']):
                score_tianjin[row['专业名称']] = []
            score_tianjin[row['专业名称']].append(row['总分'])
        if row['是否为985'] == '是':
    ...
    for key, value in score_tianjin.items():
        create_chart('天津', key, value)
