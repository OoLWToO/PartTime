...
if __name__ == '__main__':
    ...
    for i in range(start, end):
        ...
        for j in range(n, 1000):
            code1 = '{:0>3}'.format(i)
            code2 = '{:0>3}'.format(j)
            url = f'https://echa.europa.eu/substance-information/-/substanceinfo/100.{code1}.{code2}'
            print(url)
            try:
            ...
            try:
                ...
                mol_folmula = ''
            print(f'{code1}{code2}   {name}   {cas_no}   {mol_folmula}')
            data['number'].append(f'{code1}{code2}')
            data['Name'].append(name)
            data['CAS_no'].append(cas_no)
            data['Mol.formula'].append(mol_folmula)
            df = pd.DataFrame(data)
            code1 = '{:0>3}'.format(start)
            code2 = '{:0>3}'.format(start_num)
            df.to_excel(f'substance({code1}{code2}).xlsx', index=False)
